...
class GlobalSearch(BaseSearch):
    ...
    ) -> tuple[str | list[str] | dict[str, str], str | list[pd.DataFrame] | dict[str, pd.DataFrame]]:
    ...
    def search_with_context(
        self,
        query: str,
        context_chunks: str | list[str] | dict[str, str],
        context_records: str | list[pd.DataFrame] | dict[str, pd.DataFrame],
        callback: BaseLLMCallback,
        conversation_history: ConversationHistory | None = None,
        **kwargs: Any,
    ) -> GlobalSearchResult:
        return asyncio.run(self.asearch_with_context(query, context_chunks, context_records, callback, conversation_history))
    ...
    def search(
        ...
        **kwargs: Any,
        ...
        return asyncio.run(self.asearch(query, callback, conversation_history))
    ...
    async def _map_response_single_batch(
        ...
        **llm_kwargs,
        ...
        search_prompt = ""
        try:
            context_data = context_data.encode("raw_unicode_escape").decode('unicode-escape')
        except:
            pass
        # print(context_data)
        try:
            ...
            # print("MAP SEARCH PRMOPT: %s", search_prompt)
            ...
            # print(f"数据: {search_prompt} \n 回答：{processed_response}")
    ...
    def parse_search_response(self, search_response: str) -> list[dict[str, Any]]:
        ...
        return [
            ...
            for element in parsed_elements if int(element["score"]) >= 0
        ]
    ...
    async def _reduce_response(
        ...
        **llm_kwargs,
        ...
        try:
            ...
            filtered_key_answer_dict = {}
            filtered_key_points_new = []
            for filtered_key_point in filtered_key_points:
                filtered_key_answer = filtered_key_point["answer"].split("[")[0]
                if not filtered_key_answer_dict.get(filtered_key_answer, 0):
                    filtered_key_points_new.append(filtered_key_point)
                    filtered_key_answer_dict[filtered_key_answer] = 1
            filtered_key_points = filtered_key_points_new

            filtered_key_points = filtered_key_points[:20] # Only take top 20
            ...
            text_data = text_data.encode("raw_unicode_escape").decode('unicode-escape')
            ...
            print("REDUCE SEARCH PROMPT: %s", search_prompt)

            search_response = await self.llm.agenerate(
                search_messages,
                streaming=True,
                callbacks=[callback],  # type: ignore
